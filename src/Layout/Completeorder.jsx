...
export default function Completeorder() {
  ...
  // Helper to calculate numeric balance
  const getBalance = (order) => {
    if (order.balance != null) return order.balance;
  ...
  const getAdvancePaid = (order) => {
    if (order.advance != null) return parseFloat(order.advance) || 0;
    if (order.paidAmount != null) return parseFloat(order.paidAmount) || 0;
    return 0;
  };
      ...
      {loading ? (
        ...
        <div className="flex flex-col gap-6 mt-6 pb-20 max-w-5xl mx-auto px-4">
          {orders.map((order, index) => {
            const patientName = order.patientName || order.name || "Customer";
            ...
            const whatsappMessage =
              ...
              `*Total Amount:* ${formatCurrency(totalAmt, currencySymbol)}\n` +
              `*Paid Amount:* ${formatCurrency(paidAmt, currencySymbol)}\n` +
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-5">
                    ...
                    <div className="flex gap-2 sm:gap-4 items-center flex-wrap">
                      {order?.whatsappNumber ? (
                        ...
                        >
                          <FaPhoneAlt className="cursor-pointer hover:scale-110 transition w-7 h-7 sm:w-7 sm:h-7" />
                        </a>
                      ) : (
                        <FaPhoneAlt className="text-[#007A3F] cursor-not-allowed opacity-50 w-7 h-7 sm:w-7 sm:h-7" />
                      )}

                      <MdRemoveRedEye
                        className="text-[#019AF8] cursor-pointer hover:scale-110 transition w-7 h-7 sm:w-7 sm:h-7"
                        onClick={() => {
                      ...
                      <MdEdit
                        className="text-[#FF9101] cursor-pointer hover:scale-110 transition w-7 h-7 sm:w-7 sm:h-7"
                        onClick={() =>
                      ...
                      {order?.whatsappNumber ? (
                        ...
                        <svg
                          viewBox="0 0 32 32"
                          className="opacity-50 flex-shrink-0 w-7 h-7 sm:w-7 sm:h-7"
                        >
                          <path
                            fill="#25D366"
                            d="M16 .667C7.64.667.667 7.64.667 16c0 2.82.735 5.555 2.132 7.963L0 32l8.315-2.745A15.26 15.26 0 0016 31.333C24.36 31.333 31.333 24.36 31.333 16S24.36.667 16 .667z"
                      ...
                      <RiDeleteBinLine
                        className="text-[#FF0000] cursor-pointer hover:scale-110 transition w-7 h-7 sm:w-7 sm:h-7"
      ...
      {showSlip && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-center items-center z-50">
          <div className="bg-white max-h-[90vh] w-[90%]  rounded-xl shadow-xl overflow-y-auto relative p-4">
            ...
            {selectedOrder ? (
              <Customerorder order={selectedOrder} showDelivered={true} />
            ) : (
